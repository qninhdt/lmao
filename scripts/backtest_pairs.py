...
def run_backtest_for_pair(pair: str, temp_config_path: Path) -> Path | None:
    ...
    command = [
        ...
        "--export",
        "trades",
        # Specify export directory so .last_result.json is in the right place
        "--export-directory",
    ...
    try:
        ...
        pair_data = {
            ...
            "total_profit_pct": round(
                pair_specific_stats.get("profit_total_pct", 0.0), 2
            ),
            "profit_total_abs": round(strategy_stats.get("profit_total_abs", 0.0), 3),
            "market_change_pct": round(
                strategy_stats.get("market_change", 0.0) * 100, 2
            ),
        ...
        df = (
            pd.read_csv(CSV_OUTPUT_PATH) if CSV_OUTPUT_PATH.exists() else pd.DataFrame()
        )
        ...
        print(f"❌ Critical error processing file {zip_file_path.name}: {e}")
    finally:
