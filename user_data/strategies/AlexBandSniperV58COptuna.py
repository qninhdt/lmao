...
class RealOptunaManager:
    ...
    def _log_formatted_parameters(self, pair: str, params: Dict[str, Any]):
        ...
        logger.info(f"│  • min_strong_divergence_count: {params.get('min_strong_divergence_count', 'N/A')}")
        logger.info(f"│  • divergence_freshness_periods: {params.get('divergence_freshness_periods', 'N/A')}")
        ...
        logger.info(f"│  • volume_breakout_rsi_min: {params.get('volume_breakout_rsi_min', 'N/A')}")
        logger.info(f"│  • volume_breakout_rsi_max: {params.get('volume_breakout_rsi_max', 'N/A')}")
        ...
        logger.info(f"│  • emergency_profit_limit: {params.get('emergency_profit_limit', 'N/A')}")
        ...
        logger.info(f"│  • session_multiplier_quiet: {params.get('session_multiplier_quiet', 'N/A')}")
        ...
        logger.info(f"│  • mr1_rsi_exit_short: {params.get('mr1_rsi_exit_short', 'N/A')}")
        ...
        logger.info(f"│  • mc1_momentum_break_threshold: {params.get('mc1_momentum_break_threshold', 'N/A')}")
        ...
        logger.info(f"│  • vb1_min_volatility: {params.get('vb1_min_volatility', 'N/A')}")
        ...
        logger.info(f"│  • reversal_min_profit_long: {params.get('reversal_min_profit_long', 'N/A')}")
        logger.info(f"│  • reversal_min_profit_short: {params.get('reversal_min_profit_short', 'N/A')}")
        logger.info(f"│  • rsi_exit_overbought: {params.get('rsi_exit_overbought', 'N/A')}")
...
class AlexBandSniperV58CO(IStrategy):  # CHANGED: Klassenname von AlexBandSniperV51C zu AlexBandSniperV51COptuna
    """
    ...
    def __init__(self, config: dict):
        ...
        try:
            self.optuna_manager = RealOptunaManager("AlexBandSniperV58CO")  # CHANGED: Strategy name angepasst
        ...
        self.enable_dynamic_optimization = True  # ADDED: Dynamische Optimierung aktiviert
    ...
    def create_objective_function(self, pair: str):
        ...
        def objective(trial):
            params = {
                ...
                'mr1_max_hold_minutes': trial.suggest_int('mr1_max_hold_minutes', 25, 50),
                'mr1_rsi_exit_long': trial.suggest_float('mr1_rsi_exit_long', 62.0, 75.0),
                'mr1_rsi_exit_short': trial.suggest_float('mr1_rsi_exit_short', 25.0, 38.0),
                'mr1_quick_profit_target': trial.suggest_float('mr1_quick_profit_target', 0.015, 0.035),
                'mr1_timeout_min_profit': trial.suggest_float('mr1_timeout_min_profit', 0.005, 0.015),
            ...
            for pair in all_pairs:
                try:
                    if self.should_retrain_pair(pair, current_time):
                        ...
                        self.optuna_manager.optimize_coin(pair, self.create_objective_function(pair), n_trials=15)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        try:
            ...
            informative['willr'] = ta.WILLR(informative, timeperiod=self.willr_period.value)
        ...
        try:
            ...
            bollinger = qtpylib.bollinger_bands(informative['close'], window=self.bb_period.value, stds=self.bb_std.value)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        bullish_divergence = (
            ...
            (dataframe['total_bullish_divergences_count'].shift(1) >= coin_params.get('min_divergence_count', self.min_divergence_count.value))
        )

        bearish_divergence = (
            (dataframe['total_bearish_divergences'].shift(1) > 0) &
            (dataframe['total_bearish_divergences_count'].shift(1) >= coin_params.get('min_divergence_count', self.min_divergence_count.value))
        )
        ...
        market_trending = (
            dataframe['chop'].shift(1) < coin_params.get('chop_threshold', 61.8)
        )

        # Price position filters for better entry timing
        good_long_position = (
            (dataframe['bb_percent'].shift(1) < coin_params.get('bb_percent_long_max', 0.25)) |
            (dataframe['price_position'].shift(1) < coin_params.get('price_position_long_max', 0.3))
        )

        good_short_position = (
            (dataframe['bb_percent'].shift(1) > coin_params.get('bb_percent_short_min', 0.75)) |
            (dataframe['price_position'].shift(1) > coin_params.get('price_position_short_min', 0.7))
        )

        # === OPTIMIZED RSI TIMING ===
        rsi_long_timing = (
            (dataframe['rsi'].shift(1) > coin_params.get('rsi_long_lower_bound', 25)) &
            (dataframe['rsi'].shift(1) < coin_params.get('rsi_long_upper_tight', 65.0)) &
            (dataframe['rsi'].shift(1) > dataframe['rsi'].shift(coin_params.get('rsi_recovery_periods', 3)))
        )
        ...
        rsi_short_timing = (
            ...
            (dataframe['rsi'].shift(1) < dataframe['rsi'].shift(coin_params.get('rsi_decline_periods', 3)))
        )
        fresh_periods = coin_params.get('divergence_freshness_periods', 5)
        strong_bull_divergence = (
            (dataframe['total_bullish_divergences_count'].shift(1) >= coin_params.get('min_strong_divergence_count', 2)) &
            bullish_divergence &
        ...
        strong_bear_divergence = (
            (dataframe['total_bearish_divergences_count'].shift(1) >= coin_params.get('min_strong_divergence_count', 2)) &
            bearish_divergence &
        ...
        volume_breakout_long = (
            (dataframe['volume'].shift(1) > dataframe['volume_sma'].shift(1) * coin_params.get('volume_breakout_multiplier', 2.5)) &
            (dataframe['rsi'].shift(1) > coin_params.get('volume_breakout_rsi_min', 45)) &
            (dataframe['rsi'].shift(1) < coin_params.get('volume_breakout_rsi_max', 60)) &
        ...
        short_condition_reversal = (
            (sum(overbought_conditions) >= coin_params.get('min_overbought_conditions', 3)) & 
            (sum(short_reversal_signals) >= coin_params.get('min_reversal_signals', 2)) & 
        ...
        long_condition_trend = (
            ...
            (dataframe['volume'].shift(1) > dataframe['volume'].shift(1).rolling(20).mean() * coin_params.get('volume_trend_multiplier', 1.5)) &  # CHANGED
            (dataframe['rsi'].shift(1) > 40) &
            (dataframe['rsi'].shift(1) < 60) &
            (dataframe['close'].shift(1) > dataframe['close'].shift(3)) &
            (dataframe['atr'].shift(1) > dataframe['atr'].shift(1).rolling(20).mean() * 1.0) &
            (dataframe['close'].shift(1) > dataframe['kc_middleband'].shift(1)) &
            (dataframe['adx'].shift(1) > coin_params.get('adx_trending_min', 25)) &  # CHANGED
            has_volume
        )
        ...
        long_condition_momentum = (
            ...
            (dataframe['volume'].shift(1) > dataframe['volume'].shift(1).rolling(10).mean() * coin_params.get('volume_momentum_multiplier', 1.2)) &  # CHANGED
        ...
        long_condition_tertiary = (
            ...
            (dataframe['volume'].shift(1) > dataframe['volume'].shift(1).rolling(10).mean()) &
            has_volume
        )
        ...
        long_condition_quaternary = (
            ...
            (dataframe['volume'].shift(1) > dataframe['volume'].shift(1).rolling(5).mean() * coin_params.get('volume_momentum_multiplier', 1.2)) &  # CHANGED
            has_volume
        )
        ...
        volume_breakout_long_signal = volume_breakout_long & (dataframe['enter_tag'] == "")
        volume_breakout_short_signal = volume_breakout_short & (dataframe['enter_tag'] == "")
        ...
        mean_reversion_long_signal = mean_reversion_long & (dataframe['enter_tag'] == "")
        mean_reversion_short_signal = mean_reversion_short & (dataframe['enter_tag'] == "")
        ...
        momentum_continuation_long_signal = momentum_continuation_long & (dataframe['enter_tag'] == "")
        momentum_continuation_short_signal = momentum_continuation_short & (dataframe['enter_tag'] == "")
        ...
        if len(dataframe) > 0:
            ...
            if latest.get('enter_long', 0) == 1 or latest.get('enter_short', 0) == 1:
                ...
                logger.info(f"   📈 Signal Strength: {latest.get('signal_strength', 0)}")
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        return dataframe
    def dynamic_trailing_stop(self, pair: str, trade: Trade, current_time: datetime, 
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime',
        ...
        try:
            ...
            if not dataframe.empty:
                ...
                if len(dataframe) >= 10:
                    momentum_score = (last_row['close'] - dataframe.iloc[-10]['close']) / dataframe.iloc[-10]['close']
        except Exception:
            pass
        ...
        session_multiplier = coin_params.get('session_multiplier_overlap', 1.3) if is_overlap_session else \
                            coin_params.get('session_multiplier_major', 1.15) if is_major_session else \
                            coin_params.get('session_multiplier_quiet', 0.85)
        
        volatility_multiplier = min(2.5, max(0.6, volatility * coin_params.get('volatility_sensitivity', 40)))
        ...
        if 'MR1' in entry_tag:
            ...
            if trade_duration_minutes >= 8 and current_profit >= mr1_quick_profit * session_multiplier:
            ...
            if trade_duration_minutes >= 15 and current_profit >= 0.012:
                if not trade.is_short and bb_percent >= coin_params.get('mr1_bb_exit_long', 0.7):
                    return log_and_exit("mr1_bb_reversion")
                elif trade.is_short and bb_percent <= coin_params.get('mr1_bb_exit_short', 0.3):
                    return log_and_exit("mr1_bb_reversion")
        ...
        # === MOMENTUM CONTINUATION (MC1) EXITS ===
            ...
            mc1_profit_target = coin_params.get('mc1_profit_target', 0.04) * session_multiplier * volatility_multiplier
        ...
        # === VOLUME BREAKOUT (VB1) EXITS ===
            ...
            vb1_profit_target = coin_params.get('vb1_profit_target', 0.03) * session_multiplier
        ...
        # === TREND FOLLOWING EXITS ===
            ...
            trend_profit_target = coin_params.get('trend_profit_target', 0.045) * session_multiplier
            ...
            try:
                if dataframe is not None and len(dataframe) >= trend_ema_break and trade_duration_minutes >= 20:
                    current_ema20 = dataframe.iloc[-1]['ema20']
                    past_ema20 = dataframe.iloc[-(trend_ema_break+1)]['ema20']
                    
                    if not trade.is_short and current_ema20 < past_ema20 and current_profit >= 0.015:
                        return log_and_exit("trend_ema_break_down")
                    elif trade.is_short and current_ema20 > past_ema20 and current_profit >= 0.015:
            ...
            try:
                if dataframe is not None and not dataframe.empty:
                    ...
                    if not trade.is_short and last_row.get('enter_short', 0) == 1:
                        if current_profit >= coin_params.get('reversal_min_profit_long', 0.008):
                            return log_and_exit("reversal_short_signal")
                    
                    if trade.is_short and last_row.get('enter_long', 0) == 1:
                        if current_profit >= coin_params.get('reversal_min_profit_short', 0.008):
                            return log_and_exit("reversal_long_signal")
            except:
                pass

        # === RSI EXTREME EXITS ===
        if trade_duration_minutes >= 15 and current_profit >= coin_params.get('rsi_extreme_min_profit', 0.015):
        ...
        if trade_duration_minutes >= 15 and current_profit >= coin_params.get('momentum_fade_min_profit', 0.012):
            fade_threshold = coin_params.get('momentum_fade_threshold', 0.02) * volatility_multiplier
