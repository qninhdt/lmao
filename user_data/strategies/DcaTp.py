...
class DcaTp(IStrategy):
    ...
    def on_trade_open(self, trade: Trade, **kwargs) -> None:
        flags = {
            ...
            "last_dca_time": None,
            "dca_reduce_done": False,
            "open_reduce_done": False,
            "need_rebuy": False,
            "last_tp_time": None,
            "low_margin_start": None,
            "trend_level": 0,
            "bottom_added": False,
            "top_reduced": False,
            "bb_added": False,
            "pullback_ready": True,
            "reset_needed": False,
            "last_trend_side": "long",
            "last_fallback_price": None,
            "fallback_repull_done": False,
            "tp_repull_done": False,
            "last_floating_tp_price": None,
            "floating_tp_repull_done": False,
            "bottom_add_state": None,
        }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if not df30.empty:
            ...
            for name, series in series_map.items():
                dataframe[name] = (
                    pd.Series(series, index=df30.index).reindex(dataframe.index).ffill()
                )
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # 抄底入场
        long_cond = (dataframe["close"] < dataframe["bb_lowerband"]) & (
            dataframe["rsi"] < 35
        )
        ...
        **kwargs,
        ...
        if not dca_done and current_rate <= threshold and last_rsi < rsi_thresh:
            ...
            trade.set_custom_data(
                "last_action_min", int(current_time.timestamp() // 60)
            )
        ...
        if dca_block:
            logger.debug(
                f"[{trade.pair}] skip tp_repull because recent DCA blocked repull"
            )
        ...
        if (
            ...
            and fallback_ready
            and not done
            and price <= last_fb_price * 0.99
        ):
            if dca_block:
                logger.debug(
                    f"[{trade.pair}] skip fallback repull because recent DCA blocked repull"
                )
            else:
        ...
        roi_target = 0.0
        for k, v in sorted(
            self.minimal_roi_user_defined.items(), key=lambda x: int(x[0]), reverse=True
        ):
            if elapsed >= int(k):
                roi_target = v
                break
        if current_profit >= roi_target:
            if u > 0:
                ...
                if (
                    not last_tp
                    or Timestamp(last_tp, unit="s").floor("min") != candle_ts
                ):
        ...
        if (
            (not trade.get_custom_data("bottom_added"))
            and last["j_30"] < 0
            and last["rsi"] < 35
        ):
        ...
        if (
            (not trade.get_custom_data("top_reduced"))
